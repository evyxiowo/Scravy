...
export async function ExecuteWorkflow(executionId: string, nextRunAt?:Date) {
  ...
  await cleanupEnvironments(environment);
...
async function executePhase(
  ...
  if (!runFn) {
    logCollector.error(`not found executor ${node.data.type}`)
    return false;
  }
...
function setupEnvironmentForPhase(
  ...
  for (const input of inputs) {
    ...
    if (!connectedEdge) {
      console.error(`Missing input: ${input.name}`);
      continue;
    }
    const outputValue =
...
async function cleanupEnvironments(environment: Environment) {
...
async function decrementCredits(
  ...
  try {
    ...
    console.error(error);
    logCollector.error("Insufficient balance update");
    return false;
  }
}
